...
export default function ArticleEditor({ categories, tags }) {
    ...
    // Restore from API or localStorage on mount
    useEffect(() => {
        const loadDraft = async () => {
            try {
                // 1. Try to load draft from API
                // TODO: Implement API call to get draft articles
                // const draftResponse = await fetch(`${API_URL}/articles?status=draft&author=current`);
                // if (draftResponse.ok) {
                ...
                //         setContent(draft.content || '# Mon Article\n\nÉcrivez votre contenu ici...');
                //         setSelectedTags(draft.tags || []);
                //         setSelectedCategory(draft.category || categories[0]);
                //         setCoverPreview(draft.cover || '');
                //         return; // Exit early if API draft found
                //     }
                // }
                ...
                if (savedDraft) {
                    ...
                    // Only restore if draft is less than 1 year old
                    const daysSinceSave = (Date.now() - draftData.timestamp) / (1000 * 60 * 60 * 24);
                    if (daysSinceSave < 365) {
                        setTitle(draftData.title || '');
                        setDescription(draftData.description || '');
                        setContent(draftData.content || '# Mon Article\n\nÉcrivez votre contenu ici...');
                        setSelectedTags(draftData.selectedTags || []);
                        setSelectedCategory(draftData.selectedCategory || categories[0]);
                        setCoverPreview(draftData.coverPreview || '');
                        return; // Exit if localStorage draft found
                    }
                }

                // 3. Use default values (already set in initial state)
            } catch (error) {
    ...
    const handleSubmit = async (e, submitStatus = 'published') => {
        ...
        try {
            ...
            let coverImageKey = '';
            if (coverImage) {
                coverImageKey = await uploadImage(coverImage);
            }
